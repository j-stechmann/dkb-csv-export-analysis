...
_Last reviewed against v1.9.1 (dev OIDC provider added)._
...
| Target                                     | What it does                                                                                                                                                                                                                                                                                                                                                  |
| ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `make dev`                                 | Health-checks `:8080`, starts llama-server if needed, starts the dev OIDC provider if needed, installs deps, runs `bun dev` in the foreground; a bash trap stops llama-server on Ctrl-C **only if this invocation started it** (tracked via `/tmp/llama-server.managed`). The Authentik containers are stateful and left running — `make oidc-stop` when done |
...
| `make oidc-stop`                           | `docker compose stop` (not `down -v`): the `authentik-db` named volume keeps the provisioned client                                                                                                                                                                                                                                                           |
