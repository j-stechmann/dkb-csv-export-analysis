...
Five tables ([lib/db/schema.ts](../lib/db/schema.ts)):
...
| `iban`       | TEXT NOT NULL | unique; uppercased at parse time      |
...
| `nameKey`     | TEXT       | normalized lookup key: trim, lowercase, collapsed spaces; unique                                                |
...
Unique index **`(payer, payee, counterparty_iban)`** makes matching
at-most-one-rule-per-triple by construction. See
