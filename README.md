...
**Label rules (deterministic suggestions):** manually assigning a label to a
transaction in the transactions table learns a rule from its exact
counterparty combination (payer + payee + IBAN). Future transactions of the
same combination carry a `rule_label` in the LLM prompt — a human-made rule
the model must reuse verbatim unless it clearly doesn't fit. This gives
deterministic labels for recurring counterparties while the LLM stays in the
loop and may deviate when a transaction contradicts the rule.
...
- Learned rules are listed per label (click the rule badge) and can be
  edited, applied to existing transactions and deleted individually;
  "Anwenden" resets every transaction matching the exact
  payer/payee/IBAN combination so the worker re-labels it with the rule
